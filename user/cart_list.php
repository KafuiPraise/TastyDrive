		...
		<div class="row">
			<div class="col-lg-8">
				...
				if (isset($_SESSION['login_user_id'])) {
					...
					$stmt = $conn->prepare("SELECT *, c.id as cid FROM cart c INNER JOIN product_list p ON p.id = c.product_id WHERE c.user_id = ?");
					$stmt->bind_param("s", $user_id);
				} else {
					$ip = isset($_SERVER['HTTP_CLIENT_IP']) ? $_SERVER['HTTP_CLIENT_IP'] : (isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR']);
					$stmt = $conn->prepare("SELECT *, c.id as cid FROM cart c INNER JOIN product_list p ON p.id = c.product_id WHERE c.client_ip = ?");
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-md-4">
									...
									<p> <b><small>Unit Price :<?php echo "GH₵" . number_format($row['price'], 2) ?></small></b></p>
									<p><small>Qty :</small></p>
								...
								<div class="col-md-4 text-right">
									<b>
										<large><?php echo "GH₵" . number_format($row['qty'] * $row['price'], 2) ?></large>
									</b>
								</div>
							</div>
						</div>
				...
				<div class="sticky">
					<div class="card">
						<div class="card-body">
							...
							<hr>
							<p class="text-right"><b><?php echo "GH₵" . number_format($total, 2) ?></b></p>
							<hr>
