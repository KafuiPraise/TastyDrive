					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php
								$i = 1;
								$stmt = $conn->prepare("SELECT * FROM product_list ORDER BY id ASC");
								$stmt->execute();
								...
								?>
									<tr>
										...
										<td class="text-center">
											<img src="<?php echo isset($row['img_path']) ? '../user/assets/img/' . $row['img_path'] : '' ?>" alt="" id="cimg">
										</td>
										<td class="">
...
<script>
	...
	$('.edit_menu').click(function() {
		...
		cat.find("#cimg").attr('src', '.' + $(this).attr('data-img_path'))
		end_load()
	})
